<div class="p-4 space-y-4">
    ...
    @php
        $empresaNombres = $empresaOptions ?? [];
        $sucursalNombres = $sucursalOptions ?? [];

        $agrupado = $detalles
            ->groupBy(fn($d) => $empresaNombres[$d->amdg_id_empresa] ?? ($d->empresa_nombre ?? $d->amdg_id_empresa))
            ->map(function ($porEmpresa) use ($sucursalNombres) {
                return $porEmpresa
                    ->groupBy(
                        fn($d) => $sucursalNombres[$d->amdg_id_sucursal] ??
                            ($d->sucursal_nombre ?? $d->amdg_id_sucursal),
                    )
                    ->map(function ($porSucursal) {
                        return $porSucursal->groupBy('proveedor_codigo');
                    });
            });
        ...
        $totalGeneral = $detalles->sum('saldo');
    @endphp
        ...
        <details class="mb-4 overflow-hidden rounded-lg border border-gray-200 dark:border-gray-700" open>
            ...
            <div class="space-y-3 bg-white p-4 dark:bg-gray-800">
                @foreach ($sucursales as $sucursal => $proveedores)
                    @php
                        $subtotalSucursal = $proveedores->flatten()->sum('saldo');
                    @endphp
                    ...
                    <details class="rounded border border-gray-100 dark:border-gray-700" open>
                        ...
                        <div class="space-y-3 p-3">
                            @foreach ($proveedores as $proveedor => $facturas)
                                @php
                                    $subtotalProveedor = $facturas->sum('saldo');
                                    $firstConNombre = $facturas->first(fn($f) => filled($f->proveedor_nombre));
                                    $firstConRuc = $facturas->first(fn($f) => filled($f->proveedor_ruc ?? null)); // si existe en tu modelo
                                            ...
                                            <table class="w-full text-left text-sm text-gray-500 dark:text-gray-400">
                                                <thead
                                                    ...
                                                    <tr>
                                                        ...
                                                        <th scope="col" class="px-4 py-2 text-right">Saldo</th>
                                                    </tr>
                                                </thead>
                                                <tbody>
                                                    @foreach ($facturas as $index => $detalle)
                                                        <tr
                                                            ...
                                                            <td class="px-4 py-2 text-right">
                                                                ${{ number_format((float) $detalle->saldo, 2) }}
                                                            </td>
                                                        </tr>
                                                    @endforeach
                                                </tbody>
